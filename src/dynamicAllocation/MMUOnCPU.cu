...
void updateSubtree(bool tree[], int idx)
{
    //check the boundary
    if (idx >= MAX_SUBTREE_SIZE) {
        return;
    }
    ...
    //Update the left and right child nodes recursively
    updateSubtree(tree, 2 * idx + 1);
    updateSubtree(tree, 2 * idx + 2);
}

//find the free node for subtree algorthmn
int findFirstFreeNode(bool FreeNodeArray[], int height) 
{
    ...
    int start = (1 << (height - 1)) - 1;  // 2^(height - 1) - 1
    int end   = (1 << height) - 2;       // 2^height - 2

    //check the boundary
    if (end >= MAX_SUBTREE_SIZE) {
        end = MAX_SUBTREE_SIZE - 1;
    }

    //find the first node
    for (int i = start; i <= end; i++) {
        if (FreeNodeArray[i]) {  // find the free
            return i;
        }
    }
...
//the protion size is expanded to the nearest size to 2 power
unsigned int expandToPow2_Math(unsigned int portionSize)
{
    // portionSize is zero
    if (portionSize == 0) {
        return 1;
    ...
    // check if the portion size is power to 2
    if ((portionSize & (portionSize - 1)) == 0) {
        return portionSize; //if true, directly return
    }

    //calculate the ceiling
    double exponent = ceil(log2((double)portionSize));

    //bit calculation for  2^exponent
    unsigned int newSize = 1u << (unsigned int)exponent;
    return newSize;
}

//allocate the portion to the Portion Blocks
void* AllocatePortionWithSubtree(bool FreeNodeArray[], int protionSize, void* startAddress)
{
    // the protion size is expanded to the nearest size
    unsigned int expandedSize = expandToPow2_Math(protionSize);
    unsigned int heightWithpadding = int(log2((double)expandedSize) + 1);
    
    //find the free nodes
    int freeNodeIndex = findFirstFreeNode(FreeNodeArray, heightWithpadding);
    
    //update children nodes recursively
    updateSubtree(FreeNodeArray, freeNodeIndex);

    return startAddress+freeNodeIndex*64*1024;
}
