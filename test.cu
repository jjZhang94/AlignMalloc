...
__global__ void getHWPrefetch(int n, char *x, int device)
{
  clock_t cycles = 1000;
...
int main(void)
{
  ...
  printf("x[0*BB]: 0x%llx , x[1*BB]: 0x%llx, x[2*BB]: 0x%llx, x[4*BB]: 0x%llx, x[8*BB]: 0x%llx, x[16*BB]: 0x%llx,,,,%d %d %d %d %d %d\n", 
          x+0*BB, x+1*BB, x+2*BB, x+4*BB, x+8*BB, x+16*BB, *(x+0*BB), *(x+1*BB), *(x+2*BB), *(x+4*BB),*(x+8*BB), *(x+16*BB));
...
nvcc -g -Wno-deprecated-gpu-targets getHWPrefetcherP1.cu -lcudart -o getHWPrefetcherP1
**
