...
#define NODEPADDING 64  //Each node has an independence padding 
...
#define ADJACENCYLISTNUMBER 8192 // Each node has the maximum number, this also can be read from dynamic allocation
...
int main(void)
{
    ...
    char *adjacencyAddress = (char*)malloc( N*sizeof(char));
    char *d_adjacencyAddress;

    cudaMalloc(&d_adjacencyAddress, N*sizeof(char));
    ...
    for(int i = 0; i < BLOCKNUMBER*BLOCKSIZE; i++)
    {
        for(int j = 0; j < ADJACENCYLISTNUMBER; j++)
        {
            ...
            *startAddressInt = rand() % (NODENUMBER);
            //set weight
    ...
    // Copy the index array to the device
    cudaMemcpy(d_adjacencyAddress, adjacencyAddress,  N*sizeof(char), cudaMemcpyHostToDevice);

    gettimeofday(&start, NULL);
    randomWalk<<<BLOCKNUMBER, BLOCKSIZE>>>(d_adjacencyAddress, d_indexSelect);
